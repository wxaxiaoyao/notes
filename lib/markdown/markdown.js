...
function block_code(obj) {
	...
	htmlContent = html_escape(htmlContent);
...
function header(obj) {
	...
	var id = content;
	//var id = content.replace(/['",.|\(\)\[\]\{\}`@]/g, "").replace(/\s/g, '-'); // 过滤特殊字符
	var htmlContent = '<' + tag + ' id="'+ id + '">' + obj.md.line_parse(content) + '</' + tag + '>';
	if (hn_render) {
...
function htmlcode(obj, env) {
	...
	var text = cur_line, i = obj.start + 1, single_line = cur_line;
	let isSingleLine = false;
	if (/^@<@!.*@>$/.test(cur_line) || /^@<.*@\/@>$/.test(cur_line)) {
		// 是注释 或单行代码
		isSingleLine = true;
	}
	for (i = obj.start + 1; !isSingleLine && i < obj.lines.length; i++) {
		var line = obj.lines[i];
...
function paragraph(obj, env) {
	...
	if (paragraph_render) {
		token.htmlContent = paragraph_render({md:obj.md, content: text, text:text})  || token.htmlContent;
	}
...
function list(obj) {
	...
	var is_list = function(line) {
		...
		if (line.match(/^\d+@\. /)) {
			return {is_list:true, is_sort: true, value: parseInt(line.substring(0, line.indexOf("@.")))};
		}
...
function render_token(token) {
	...
	htmlContent += "<" + token.tag + (token.attrs ? " " + token.attrs : "")  + ">";
