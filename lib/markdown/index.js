...
export const markdownEx = function(options) {
	...
	md.parseBlock = function (block) {
		...
		var text = token.text;
		var line = text.split("\n")[0];
		var isMarkdownsMod = token.tag == "pre"  && /^```@([\w_\/]+)/.test(line);

		if (!isMarkdownsMod) {
			//block.blockUrl = undefined;
			block.isTemplate = false;
			block.modName = "markdowns";
			block.cmdName = "markdowns";
			block.styleName = "default";
			block.modParams = {text:token.text};
		} else {
			var wikiCmdRE = /^```@([\w_\/]+)/;
			...
			try {
				...
				modParams = mdconf.mdToJson(content);
				//modParams = undefined;
			}
			...
			block.modParams = typeof(modParams) == "string" ? undefined : modParams;
