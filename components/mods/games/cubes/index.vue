...
<template>
	<div class="games-cube-index-container">
		<div class='state'>
			<div id='startBtn' @click="start" v-show="isStart">Start</div>

			<div id='cubeWrap'>
				<div id='floorWrap' class='floor-wrap'></div>
			</div>
		...
		<!-- 计时 -->
		<div class='time-area' id='time' :style="timeStyle">
			00:00
		</div>

		<!-- 成功 -->
		<div class='success' id='successTxt' v-show="isSuccess">
			<span>YOU WIN</span>
		</div>
...
export default {
	...
	methods: {
		...
		start() {
			this.isStart = true;
			this.timeStyle.top = "10px";

			_.each(this.startCallbacks, val => val());
		}

		reset() {
			this.isStart = false;
			this.timeStyle.top = "-30px";

			_.each(this.resetCallbacks, val => val());
		}
	},
...
.games-cube-index-container {width: 100%;height: 100%; position: absolute; -webkit-user-select: none; -moz-user-select: none; font-family: impact;}
...
.cool .block.block-front:after {position: absolute; background: rgba(251,245,0,0.7);}
.cool .block.block-right:after {position: absolute; background: rgba(56,123,222,0.7);}
.cool .block.block-back:after {position: absolute; background: rgba(218,35,28,0.7);}
.cool .block.block-left:after {position: absolute; background: rgba(0,146,63,0.7);}
.cool .block.block-up:after {position: absolute; background: rgba(251,253,254,0.7);}
.cool .block.block-bottom:after {position: absolute; background: rgba(227,120,21,0.7);}
