	...
	<div class="pagenavs-container container">
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item v-for="(path, index) in paths" :key="index" v-if="index != 0">
				<el-popover trigger="hover" @show="showPathItems(index)" v-loading="itemsLoading && pathIndex == index">
...
<script>
		//<a class="iconfont icon-edit editor-link" :href="'/note/editor#' + url" style="cursor:pointer" target="_blank"></a>
import {
	Breadcrumb,
	BreadcrumbItem,
	Popover,
...
export default {
	...
	methods: {
		...
		clickPathItem(page, index){
			...
			//setTimeout(() => this.isShowPopover[index] = false);
		}
	},
